...
use super::{
    ...
    manifest::{ComponentType, Concept, Manifest, NamespaceOrOther, Namespace},
};
use anyhow::Context;
...
impl Tree {
    ...
    pub fn to_token_stream(
        ...
        .to_token_stream(components_tree, api_name, project_path)
    }

    pub fn get(&self, path: IdentifierPath) -> Option<&Concept> {
        self.root.get(path)
    }

    fn insert(&mut self, path: IdentifierPathBuf, inner: TreeNodeInner<Concept>) -> anyhow::Result<()> {
...
impl TreeNode {
        ...
        match &self.inner {
            TreeNodeInner::Namespace(ns) => {
                let children = ns
                    .children
                    .values()
                    .map(|child| child.to_token_stream(components_tree, api_name, project_path))
                    .collect::<Result<Vec<_>, _>>()?;

                let prelude = quote! {
                    use #api_name::{once_cell::sync::Lazy, ecs::{Concept, __internal_get_concept}};
                };
            ...
            TreeNodeInner::Other(concept) => {
                ...
                let make_concept = self.generate_make(components_tree, &project_path, concept)?;
                let is_concept = self.generate_is(&project_path, &concept,)?;
                Ok(quote! {
                    #make_concept
                    #is_concept
                })
    ...
    fn generate_make(
        ...
        let make_ident = quote::format_ident!("make_{}", IdentifierPathBuf::from_iter(
            identifier_path.iter().chain(self.path.iter()).cloned(),
        )
        .to_string());
    ...
    fn generate_is(&self, identifier_path: &IdentifierPath, concept: &Concept) -> anyhow::Result<TokenStream> {
        let is_comment = format!(
            "Checks if the entity is a {} ({})",
            concept.name, concept.description
        );
        let is_ident =  quote::format_ident!("make_{}", IdentifierPathBuf::from_iter(
            identifier_path.iter().chain(self.path.iter()).cloned(),
        )
        .to_string());
...
impl TreeNodeNamespace {
    ...
    fn get(&self, path: IdentifierPath) -> Option<&Concept> {
